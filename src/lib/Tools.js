...
// user's conversation via listFiles/readFile/searchFiles/searchText. This denial is
// intentionally structural -- checked before, and independent of, any `permissions`
// config -- because most ai-agent consumers do not set `permissions` at all, and a
// policy-only gate would leave them exposed. It has no effect on conversation history
// persistence itself: saveHistory/history() in Prompt.js write and read the --history
// file directly via node:fs, never through these tool wrappers, so history still works
// even when --history points inside this tree. See CSEC-027 / CSEC-028.
